...
import { asArray, expectShape } from "@/lib/safe-data";
  ...
  useEffect(() => {
    ...
    const tick = () =>
      fn()
        .then((r) => {
          if (!alive) return;
          const ok = expectShape<MissionControl>(r, ["spend", "brainHealth", "topModels", "schedules", "bridges"]);
          const usable = ok && (ok as any).spend && (ok as any).schedules && (ok as any).bridges ? ok : null;
          if (usable) { setM(usable); setErr(null); }
          else setErr("Sign-in required for live mission control");
        })
  ...
  return (
    <section className="cathedral-card rounded-xl p-5 space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Stat icon={DollarSign} label="Spend 24h"  value={`$${(m.spend?.last24hUsd ?? 0).toFixed(2)}`} />
        <Stat icon={DollarSign} label="Spend 30d"  value={`$${(m.spend?.last30dUsd ?? 0).toFixed(2)}`} />
        <Stat icon={Cpu}        label="AI calls 24h" value={m.spend?.calls24h ?? 0} />
        <Stat icon={Sparkles}   label="Intel 24h"  value={m.intel24h ?? 0} />
        <Stat icon={Radio}      label="Bridges"    value={m.bridges?.paired ?? 0} />
        <Stat icon={Calendar}   label="Schedules"  value={`${m.schedules?.enabled ?? 0}/${m.schedules?.total ?? 0}`} />
        <Stat icon={Activity}   label="Sched fails" value={m.schedules?.failures24h ?? 0} accent={(m.schedules?.failures24h ?? 0) > 0 ? "destructive" : undefined} />
        <Stat icon={Cpu}        label="Brains live" value={asArray<any>(m.brainHealth).filter((h) => h.status === "ok").length} />
      </div>

      {asArray<any>(m.topModels).length > 0 && (
        <div>
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-2">Top models · 24h</div>
          <ul className="space-y-1.5 text-sm">
            {asArray<any>(m.topModels).map((t) => (
